# ================================================================
# MOVEIT DEPLOYMENT - BULLETPROOF FIND EXISTING RESOURCES
...
Write-Log "Using VNet: $selectedVNet" "Green"
Write-Host ""

# Find subnet in that VNet
...
# Use first subnet (or one with "moveit" in name)
$selectedSubnet = $null
foreach ($subnet in $allSubnets) {
...
Write-Log "Using Subnet: $selectedSubnet" "Green"
Write-Host ""
...
Write-Log "============================================" "Cyan"
Write-Host ""
Write-Host "Network Resource Group: " -NoNewline; Write-Host $config.NetworkResourceGroup -ForegroundColor Yellow
...
az network nsg rule create --resource-group $config.NetworkResourceGroup --nsg-name $config.NSGName --name "Allow-FTPS-990" --priority 100 --direction Inbound --access Allow --protocol Tcp --source-address-prefixes '*' --source-port-ranges '*' --destination-address-prefixes '*' --destination-port-ranges 990 --output none 2>$null

az network nsg rule create --resource-group $config.NetworkResourceGroup --nsg-name $config.NSGName --name "Allow-FTPS-989" --priority 110 --direction Inbound --access Allow --protocol Tcp --source-address-prefixes '*' --source-port-ranges '*' --destination-address-prefixes '*' --destination-port-ranges 989 --output none 2>$null

az network nsg rule create --resource-group $config.NetworkResourceGroup --nsg-name $config.NSGName --name "Allow-HTTPS-443" --priority 120 --direction Inbound --access Allow --protocol Tcp --source-address-prefixes '*' --source-port-ranges '*' --destination-address-prefixes '*' --destination-port-ranges 443 --output none 2>$null
...
if (-not $lbExists) {
    ...
    az network lb probe create --resource-group $config.DeploymentResourceGroup --lb-name $config.LoadBalancerName --name "health-probe-ftps" --protocol tcp --port 990 --interval 15 --threshold 2 --output none
    
    az network lb rule create --resource-group $config.DeploymentResourceGroup --lb-name $config.LoadBalancerName --name "lb-rule-990" --protocol Tcp --frontend-port 990 --backend-port 990 --frontend-ip-name "LoadBalancerFrontEnd" --backend-pool-name "backend-pool-lb" --probe-name "health-probe-ftps" --idle-timeout 15 --enable-tcp-reset true --output none
    
    az network lb rule create --resource-group $config.DeploymentResourceGroup --lb-name $config.LoadBalancerName --name "lb-rule-989" --protocol Tcp --frontend-port 989 --backend-port 989 --frontend-ip-name "LoadBalancerFrontEnd" --backend-pool-name "backend-pool-lb" --probe-name "health-probe-ftps" --idle-timeout 15 --enable-tcp-reset true --output none
}
...
# COMPLETE
# ----------------------------------------------------------------
$ftpsPublicIP = az network public-ip show --resource-group $config.DeploymentResourceGroup --name $config.LoadBalancerPublicIPName --query ipAddress --output tsv 2>$null
