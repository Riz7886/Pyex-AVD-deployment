...
foreach ($assignment in $roleAssignments) {
    $severity = "Low"
    $issue = "Normal assignment"
    
    if ($assignment.RoleDefinitionName -eq "Owner") {
        $severity = "Critical"
        $issue = "Owner role - full access to all resources"
    } elseif ($assignment.RoleDefinitionName -eq "Contributor") {
        $severity = "High"
        $issue = "Contributor role - can create/modify/delete resources"
    } elseif ($assignment.RoleDefinitionName -eq "User Access Administrator") {
        $severity = "High"
        $issue = "Can manage access to Azure resources"
    }
...
Write-Log "Found $($findings.Count) RBAC assignments" "SUCCESS"
Export-Report -Inventory $inventory -IdleResources $idle -Findings $findings -ReportName "1-RBAC-Audit" -Format $OutputFormat
