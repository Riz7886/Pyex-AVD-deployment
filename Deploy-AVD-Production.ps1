...
param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$TargetSubscriptionId = "7EDFB9F6-940E-47CD-AF4B-04D0B6E6020F"
)
...
Write-Host ""
Write-Host "PYX HEALTH AVD DEPLOYMENT - PRODUCTION GRADE" -ForegroundColor Cyan
Write-Host ""

Write-Host "Target Subscription: $TargetSubscriptionId" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "Connected to subscription: $($ctx.Subscription.Name)" -ForegroundColor Green
    ...
    if ($KeyVaultName) {
        ...
        Connect-AzAccount -ServicePrincipal -Credential $credential -Tenant $tenantId -SubscriptionId $TargetSubscriptionId | Out-Null
        
        Write-Host "Authenticated using Service Principal" -ForegroundColor Green
    }
} catch {
    Write-Host "Key Vault setup failed, using current authentication: $($_.Exception.Message)" -ForegroundColor Yellow
    Connect-AzAccount -ErrorAction SilentlyContinue | Out-Null
    Set-AzContext -SubscriptionId $TargetSubscriptionId | Out-Null
}

Write-Host ""
Write-Host "Starting AVD Deployment..." -ForegroundColor Cyan
Write-Host ""
...
Write-Host "Company: $CompanyName | Environment: $Environment | VMs: $vmCount" -ForegroundColor White
Write-Host ""
...
Write-Host "Selecting optimal VM size for $vmCount VMs..." -ForegroundColor Cyan
$vmOptions = @(
    @{Size="Standard_D4s_v3";vCPU=4;RAM=16;Cost=188;Family="standardDSv3Family"},
    @{Size="Standard_D2s_v3";vCPU=2;RAM=8;Cost=96;Family="standardDSv3Family"},
    @{Size="Standard_B4ms";vCPU=4;RAM=16;Cost=166;Family="standardBSFamily"},
    @{Size="Standard_B2ms";vCPU=2;RAM=8;Cost=60;Family="standardBSFamily"}
)
...
foreach ($vm in $vmOptions) {
    try {
        ...
        if ($usage) {
            ...
            $need = $vmCount * $vm.vCPU
            if ($avail -ge $need) {
                $selectedVM = $vm
                Write-Host "Selected: $($vm.Size) - $($vm.RAM)GB RAM (Quota: $avail cores available)" -ForegroundColor Green
                break
            }
        } else {
            $selectedVM = $vm
...
$cost = ($vmCount * $selectedVM.Cost) + 100

Write-Host ""
Write-Host "Configuration:" -ForegroundColor Cyan
Write-Host "VM Size: $($selectedVM.Size)" -ForegroundColor White
Write-Host "VM Count: $vmCount VMs" -ForegroundColor White
Write-Host "RAM per VM: $($selectedVM.RAM)GB" -ForegroundColor White
Write-Host "vCPU per VM: $($selectedVM.vCPU)" -ForegroundColor White
Write-Host "Total Capacity: 10 concurrent users" -ForegroundColor White
Write-Host "Estimated Cost: `$$cost/month" -ForegroundColor White
Write-Host ""
...
$pw = -join ((1..24) | % { "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()"[(Get-Random -Max 70)] })
...
$rule1 = New-AzNetworkSecurityRuleConfig -Name "Deny-RDP-Internet" -Access Deny -Protocol Tcp -Direction Inbound -Priority 100 -SourceAddressPrefix Internet -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 3389
$rule2 = New-AzNetworkSecurityRuleConfig -Name "Allow-AVD-Traffic" -Access Allow -Protocol * -Direction Inbound -Priority 110 -SourceAddressPrefix VirtualNetwork -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange *
$nsg = New-AzNetworkSecurityGroup -Name $naming.NSG -ResourceGroupName $naming.RG -Location $Location -SecurityRules $rule1,$rule2 -Tag $tags -Force
...
try {
    $kv = New-AzKeyVault -Name $naming.KeyVault -ResourceGroupName $naming.RG -Location $Location -EnabledForDeployment -EnabledForDiskEncryption -Tag $tags
...
$hp = New-AzWvdHostPool -ResourceGroupName $naming.RG -Name $naming.HostPool -Location $Location -HostPoolType Pooled -LoadBalancerType BreadthFirst -PreferredAppGroupType Desktop -MaxSessionLimit 2 -Tag $tags
...
Write-Host "AVD Components" -ForegroundColor Green

Write-Host ""
Write-Host "Deploying $vmCount VMs (10-15 min)..." -ForegroundColor Yellow
...
Write-Host "VMs Deployed: $success/$vmCount" -ForegroundColor White
Write-Host "Estimated Cost: `$$cost/month" -ForegroundColor White
Write-Host "Admin Username: avdadmin" -ForegroundColor White
...
try {
    ...
    git add .
    $commitMsg = "AVD Deployment $CompanyName $Environment $ts - $success VMs"
    ...
    Write-Host "Manual push: git push origin main" -ForegroundColor Yellow
}
