...
Write-Log "Auditing cost and tagging..."

$requiredTags = @("Environment", "Owner", "CostCenter", "Project")

foreach ($rg in $inventory.Resources.ResourceGroups) {
    $missingTags = @()
    ...
    $severity = if ($missingTags.Count -eq 0) { "Low" } elseif ($missingTags.Count -le 2) { "Medium" } else { "High" }
    $issue = if ($missingTags.Count -eq 0) { "All required tags present" } else { "Missing tags: $($missingTags -join ', ')" }
    
    $findings += [PSCustomObject]@{
        ResourceGroup = $rg.ResourceGroupName
        Location = $rg.Location
        TagCount = $rg.Tags.Count
        HasEnvironment = $rg.Tags.ContainsKey("Environment")
        HasOwner = $rg.Tags.ContainsKey("Owner")
        HasCostCenter = $rg.Tags.ContainsKey("CostCenter")
        HasProject = $rg.Tags.ContainsKey("Project")
        Severity = $severity
        Issue = $issue
...
Write-Log "Found $($findings.Count) resource groups audited" "SUCCESS"
Export-Report -Inventory $inventory -IdleResources $idle -Findings $findings -ReportName "5-Cost-Tagging-Audit" -Format $OutputFormat
