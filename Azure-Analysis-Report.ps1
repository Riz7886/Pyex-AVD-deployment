...
Write-Log "Starting comprehensive Azure environment analysis..."

$findings = @()

# Analyze Resource Groups
Write-Log "Analyzing resource groups..."
...
Write-Log "Analyzing virtual machines..."
$vms = Get-AzVM
foreach ($vm in $vms) {
...
Write-Log "Analyzing storage accounts..."
...
Write-Log "Analysis complete. Total findings: $($findings.Count)"

$reportFiles = Export-ReportData -Data $findings -ReportName "Azure-Analysis-Report" -SubscriptionName $subscription.Name -Format $OutputFormat

Write-Host ""
Write-Host "Analysis Complete" -ForegroundColor Green
Write-Host "Total Items Analyzed: $($findings.Count)" -ForegroundColor Cyan
if ($OutputFormat -ne "HTML") {
    Write-Host "CSV Report: $($reportFiles.CSV)" -ForegroundColor Cyan
}
if ($OutputFormat -ne "CSV") {
    Write-Host "HTML Report: $($reportFiles.HTML)" -ForegroundColor Cyan
}
