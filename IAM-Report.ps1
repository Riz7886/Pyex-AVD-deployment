...
Write-Log "Starting IAM analysis..."

$iamFindings = @()

# Get all role assignments
Write-Log "Collecting IAM role assignments..."
$roleAssignments = Get-AzRoleAssignment

foreach ($assignment in $roleAssignments) {
    $iamFindings += [PSCustomObject]@{
        ...
        IsCustomRole = (Get-AzRoleDefinition -Id $assignment.RoleDefinitionId).IsCustom
        Subscription = $subscription.Name
        ReportDate = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    }
}

Write-Log "IAM analysis complete. Total assignments: $($iamFindings.Count)"

$reportFiles = Export-ReportData -Data $iamFindings -ReportName "IAM-Report" -SubscriptionName $subscription.Name -Format $OutputFormat

Write-Host ""
Write-Host "IAM Report Complete" -ForegroundColor Green
Write-Host "Total Role Assignments: $($iamFindings.Count)" -ForegroundColor Cyan
Write-Host "Users: $(($iamFindings | Where-Object {$_.ObjectType -eq 'User'}).Count)" -ForegroundColor Cyan
Write-Host "Groups: $(($iamFindings | Where-Object {$_.ObjectType -eq 'Group'}).Count)" -ForegroundColor Cyan
Write-Host "Service Principals: $(($iamFindings | Where-Object {$_.ObjectType -eq 'ServicePrincipal'}).Count)" -ForegroundColor Cyan
