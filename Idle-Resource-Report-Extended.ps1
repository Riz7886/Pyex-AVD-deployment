...
# Idle VMs
foreach ($vm in $inventory.Resources.VirtualMachines) {
    $vmStatus = Get-AzVM -ResourceGroupName $vm.ResourceGroupName -Name $vm.Name -Status
    $powerState = ($vmStatus.Statuses | Where-Object { $_.Code -match "PowerState" }).DisplayStatus
    ...
    if ($powerState -match "stopped|deallocated") {
        $findings += [PSCustomObject]@{
            ...
            EstimatedMonthlySavings = "Variable based on VM size"
...
# Unattached Disks
$disks = Get-AzDisk
foreach ($disk in $disks | Where-Object { $_.ManagedBy -eq $null }) {
    $findings += [PSCustomObject]@{
        ResourceType = "Disk"
        ...
        EstimatedMonthlySavings = "$5/month"
        Subscription = $subscription.Name
    }
}
...
# Unattached NICs
$nics = Get-AzNetworkInterface
foreach ($nic in $nics | Where-Object { $_.VirtualMachine -eq $null }) {
    $findings += [PSCustomObject]@{
        ...
        Status = "Unattached"
        Size = "N/A"
        Issue = "NIC not attached to any VM"
        EstimatedMonthlySavings = "$1/month"
        Subscription = $subscription.Name
    }
}
...
# Unused Public IPs
$publicIPs = Get-AzPublicIpAddress
foreach ($ip in $publicIPs | Where-Object { $_.IpConfiguration -eq $null }) {
    $findings += [PSCustomObject]@{
        ...
        Status = "Unused"
        Size = "N/A"
        Issue = "Public IP not associated with any resource"
        EstimatedMonthlySavings = "$3/month"
        Subscription = $subscription.Name
    }
}

Export-ComprehensiveReport -Inventory $inventory -CostAnalysis $costAnalysis -DetailedFindings $findings -ReportName "Idle-Resource-Report-Extended" -Format $OutputFormat
