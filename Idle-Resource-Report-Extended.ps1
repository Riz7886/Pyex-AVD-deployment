...
foreach ($ip in $publicIPs | Where-Object { $_.IpConfiguration -eq $null }) {
    $findings += [PSCustomObject]@{
        ...
        Status = "Unused"
        Size = "N/A"
        Issue = "Not associated"
        Savings = "$3/month"
        Subscription = $subscription.Name
    }
}
Export-ComprehensiveReport -Inventory $inventory -CostAnalysis $costAnalysis -DetailedFindings $findings -ReportName "Idle-Resource-Report" -Format $OutputFormat
