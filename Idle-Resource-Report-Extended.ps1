...
# Extended idle resource analysis with cost data

$subscription = Connect-AzureWithSubscription

Write-Log "Starting Idle-Resource-Report-Extended..."

$findings = @()

# Add specific logic for Idle-Resource-Report-Extended
Write-Log "Collecting data for Extended idle resource analysis with cost data..."

# Placeholder - Add specific implementation
$findings += [PSCustomObject]@{
    ResourceName = "Sample"
    Status = "Active"
    Finding = "Extended idle resource analysis with cost data executed successfully"
...
$reportFiles = Export-ReportData -Data $findings -ReportName "Idle-Resource-Report-Extended" -SubscriptionName $subscription.Name -Format $OutputFormat

Write-Host ""
Write-Host "Idle-Resource-Report-Extended Complete" -ForegroundColor Green
