...
Write-Log "Collecting audit logs (last 7 days)..."

$startTime = (Get-Date).AddDays(-7)
$activityLogs = Get-AzActivityLog -StartTime $startTime | Select-Object -First 1000

foreach ($log in $activityLogs) {
    $severity = "Low"
    $issue = "Normal activity"
    
    if ($log.Status.Value -eq "Failed") {
        $severity = "Medium"
        $issue = "Operation failed"
    }
    
    if ($log.OperationName.Value -match "delete|remove") {
        $severity = "High"
        $issue = "Delete operation"
    }
    
    if ($log.Level -eq "Critical" -or $log.Level -eq "Error") {
        $severity = "High"
        $issue = "$($log.Level) level event"
    }
...
Write-Log "Collected $($findings.Count) audit log entries" "SUCCESS"
Export-Report -Inventory $inventory -IdleResources $idle -Findings $findings -ReportName "9-AuditLog-Collection" -Format $OutputFormat
