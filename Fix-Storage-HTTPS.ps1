...
$storageAccounts = $inventory.Resources.StorageAccounts | Where-Object { !$_.EnableHttpsTrafficOnly }

if ($storageAccounts.Count -eq 0) {
    Write-Log "All storage accounts already enforce HTTPS" "SUCCESS"
    exit 0
}

$selectedAccounts = Select-ResourcesForOperation -Resources $storageAccounts -ResourceType "Storage Account" -OperationType "Enable HTTPS"

foreach ($storage in $selectedAccounts) {
    $result = Invoke-SafeOperation -OperationName "Enable HTTPS-only" -ResourceName $storage.StorageAccountName -ResourceType "Storage Account" -Impact "Medium" -Operation {
        Set-AzStorageAccount -ResourceGroupName $storage.ResourceGroupName -Name $storage.StorageAccountName -EnableHttpsTrafficOnly $true
    }
    $findings += [PSCustomObject]@{
        ResourceType = "Storage Account"
        ResourceName = $storage.StorageAccountName
        ResourceGroup = $storage.ResourceGroupName
        Operation = "Enable HTTPS-only"
...
Export-ComprehensiveReport -Inventory $inventory -CostAnalysis $null -DetailedFindings $findings -ReportName "Fix-Storage-HTTPS-Results" -Format $OutputFormat
