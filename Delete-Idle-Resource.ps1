...
if ($ReadOnly) {
    Write-Log "This script requires modification permissions. Remove -ReadOnly flag." "ERROR"
    exit 1
}

Write-Host ""
Write-Host "========================================" -ForegroundColor Yellow
Write-Host "  IDLE RESOURCE DELETION" -ForegroundColor Yellow
Write-Host "========================================" -ForegroundColor Yellow
Write-Host ""
Write-Host "This script will help you safely delete idle resources" -ForegroundColor White
Write-Host ""
...
Write-Host "  - $($disks.Count) unattached disks" -ForegroundColor White
Write-Host "  - $($nics.Count) unattached NICs" -ForegroundColor White
Write-Host "  - $($publicIPs.Count) unused public IPs" -ForegroundColor White
Write-Host ""

# Select disks to delete
if ($disks.Count -gt 0) {
    $selectedDisks = Select-ResourcesForOperation -Resources $disks -ResourceType "Unattached Disk" -OperationType "Deletion"
    foreach ($disk in $selectedDisks) {
        $result = Invoke-SafeOperation -OperationName "Delete Disk" -ResourceName $disk.Name -ResourceType "Disk" -Impact "High" -Operation {
...
# Select NICs to delete
if ($nics.Count -gt 0) {
    $selectedNICs = Select-ResourcesForOperation -Resources $nics -ResourceType "Unattached NIC" -OperationType "Deletion"
    foreach ($nic in $selectedNICs) {
        $result = Invoke-SafeOperation -OperationName "Delete NIC" -ResourceName $nic.Name -ResourceType "Network Interface" -Impact "High" -Operation {
...
# Select Public IPs to delete
if ($publicIPs.Count -gt 0) {
    $selectedIPs = Select-ResourcesForOperation -Resources $publicIPs -ResourceType "Unused Public IP" -OperationType "Deletion"
    foreach ($ip in $selectedIPs) {
        $result = Invoke-SafeOperation -OperationName "Delete Public IP" -ResourceName $ip.Name -ResourceType "Public IP" -Impact "Medium" -Operation {
...
Export-ComprehensiveReport -Inventory $inventory -CostAnalysis $null -DetailedFindings $findings -ReportName "Delete-Idle-Resource-Results" -Format $OutputFormat
