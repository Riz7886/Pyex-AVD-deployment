...
Write-Host "Found: $($disks.Count) disks, $($nics.Count) NICs, $($publicIPs.Count) IPs" -ForegroundColor Cyan
if ($disks.Count -gt 0) {
    $selectedDisks = Select-ResourcesForOperation -Resources $disks -ResourceType "Disk" -OperationType "Deletion"
    foreach ($disk in $selectedDisks) {
        $result = Invoke-SafeOperation -OperationName "Delete Disk" -ResourceName $disk.Name -ResourceType "Disk" -Operation {
...
if ($nics.Count -gt 0) {
    $selectedNICs = Select-ResourcesForOperation -Resources $nics -ResourceType "NIC" -OperationType "Deletion"
    foreach ($nic in $selectedNICs) {
        $result = Invoke-SafeOperation -OperationName "Delete NIC" -ResourceName $nic.Name -ResourceType "NIC" -Operation {
...
if ($publicIPs.Count -gt 0) {
    $selectedIPs = Select-ResourcesForOperation -Resources $publicIPs -ResourceType "Public IP" -OperationType "Deletion"
    foreach ($ip in $selectedIPs) {
        $result = Invoke-SafeOperation -OperationName "Delete IP" -ResourceName $ip.Name -ResourceType "Public IP" -Operation {
...
Export-ComprehensiveReport -Inventory $inventory -CostAnalysis $null -DetailedFindings $findings -ReportName "Delete-Results" -Format $OutputFormat
