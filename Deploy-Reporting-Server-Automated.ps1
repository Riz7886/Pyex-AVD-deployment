...
try {
    ...
    if (-not $vnet) {
        $vnet = New-AzVirtualNetwork -Name $vnetName -ResourceGroupName $rgName -Location $Location -AddressPrefix "10.0.0.0/16" -Subnet $bastionSubnet,$vmSubnet -Tag @{Company="PyxHealth"; Purpose="Reporting-Network"}
    }
    ...
    if (-not $pip) {
        $pip = New-AzPublicIpAddress -Name $pipName -ResourceGroupName $rgName -Location $Location -AllocationMethod Static -Sku Standard -Tag @{Company="PyxHealth"}
    }
    ...
    if (-not $bastion) {
        $bastion = New-AzBastion -Name $bastionName -ResourceGroupName $rgName -PublicIpAddress $pip -VirtualNetwork $vnet -Sku Standard -Tag @{Company="PyxHealth"; Purpose="Secure-Access"}
    }
    ...
    if (-not $nic) {
        $nic = New-AzNetworkInterface -Name $nicName -ResourceGroupName $rgName -Location $Location -SubnetId $vmSubnetObj.Id -Tag @{Company="PyxHealth"}
    }
    ...
    if (-not $vm) {
        ...
        $vm = New-AzVM -ResourceGroupName $rgName -Location $Location -VM $vmConfig -Tag @{Company="PyxHealth"; Purpose="Reporting-Server"; OS="Windows-Server-2022"}
    }
