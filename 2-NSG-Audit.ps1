...
foreach ($nsg in $nsgs) {
    foreach ($rule in $nsg.SecurityRules) {
        ...
        $isWildcardSource = $rule.SourceAddressPrefix -in @("*", "Internet", "0.0.0.0/0", "Any")
        $isAllowInbound = $rule.Direction -eq "Inbound" -and $rule.Access -eq "Allow"
        
        if ($isWildcardSource -and $isAllowInbound) {
            if ($rule.DestinationPortRange -in @("22", "3389", "445", "135-139")) {
                $severity = "Critical"
                $issue = "Allows public access to sensitive port $($rule.DestinationPortRange)"
            } elseif ($rule.DestinationPortRange -eq "*") {
                $severity = "Critical"
                $issue = "Allows public access to ALL ports"
            } else {
                $severity = "High"
                $issue = "Allows public inbound access"
            }
        }
...
Write-Log "Found $($findings.Count) NSG rules" "SUCCESS"
Export-Report -Inventory $inventory -IdleResources $idle -Findings $findings -ReportName "2-NSG-Audit" -Format $OutputFormat
