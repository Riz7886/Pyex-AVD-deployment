...
Write-Log "Auditing identity and AAD..."
...
foreach ($sp in $servicePrincipals) {
    ...
    try {
        ...
        if ($app -and $app.PasswordCredentials) {
            $hasSecrets = $true
            $secretExpiry = ($app.PasswordCredentials | Sort-Object EndDateTime | Select-Object -First 1).EndDateTime
        }
    } catch { }
    
    $severity = "Low"
    $issue = "Active service principal"
    
    if ($hasSecrets -and $secretExpiry -ne "N/A") {
        $daysUntilExpiry = ($secretExpiry - (Get-Date)).Days
        if ($daysUntilExpiry -lt 30) {
            $severity = "High"
            $issue = "Secret expires in $daysUntilExpiry days"
...
foreach ($user in $users) {
    ...
    $hasPrivilegedRole = $roleAssignments | Where-Object { $_.RoleDefinitionName -in @("Owner", "Contributor", "User Access Administrator") }
    
    $severity = if ($hasPrivilegedRole) { "Medium" } else { "Low" }
    $issue = if ($hasPrivilegedRole) { "Has privileged role: $($hasPrivilegedRole.RoleDefinitionName -join ', ')" } else { "Normal user" }
...
Write-Log "Found $($findings.Count) identity findings" "SUCCESS"
Export-Report -Inventory $inventory -IdleResources $idle -Findings $findings -ReportName "7-Identity-AAD-Audit" -Format $OutputFormat
