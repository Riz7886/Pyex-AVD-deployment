...
<#
AVD ULTIMATE — subscription picker + image/region resolver + QUOTA-aware sizing

NEW (per your ask):
  • Interactive VM picker (like subscription picker):
      - Pick host count (1–10) from a numbered menu
      - Pick VM size from a numbered menu
Keeps:
  • Mandatory subscription selection
  • AVD stack: RG, VNet/Subnet, NSG (deny Internet RDP, allow VNet [+ optional OnPrem]), Storage+Azure Files (FSLogix)
  • Host Pool + Desktop App Group + Workspace + RBAC hooks (Desktop Virtualization User + VM User Login)
  • Image/region resolver (pivots if needed), quota-aware size & host-count
#>
...
param(
  ...
  # Defaults still exist but will be overridden by the new interactive picker
  [ValidateRange(1,10)]
  [int]$VmCount = 10,

  # Image choices (multi-session, single-session enterprise, auto single-session)
...
function Write-Color {
  ...
  Write-Host ("[{0}] {1}" -f (Get-Date -Format "HH:mm:ss"),$Msg) -ForegroundColor $c
}

############ Helpers ############
function Get-RegionAbbrev([string]$Region){
  $m=@{"centralus"="PHC";"eastus"="PHE";"eastus2"="PHE2";"westus"="PHW";"westus2"="PHW2";"southcentralus"="PHSC";"northcentralus"="PHNC"}
  return $m[$Region.ToLower()]
}

function Ensure-Modules {
  try { $pg = Get-PSRepository -Name "PSGallery" -ErrorAction SilentlyContinue; if ($pg -and $pg.InstallationPolicy -ne "Trusted") { Set-PSRepository -Name "PSGallery" -InstallationPolicy Trusted } } catch {}
...
function Connect-AzureAuto {
  ...
  if($ctx -and $ctx.Account){ Write-Color ("Connected as: {0}" -f $ctx.Account.Id) "SUCCESS"; return }
  try{
...
function Show-SubscriptionsIndexed([array]$Subs){
...
function Parse-IndexSelection([string]$Text,[int]$Max){
  $sel = ($Text -as [string])
  if([string]::IsNullOrWhiteSpace($sel)){ return @() }
  $sel = $sel.Trim() -replace '\s',''
  $parts = $sel -split ','
  $idxs = New-Object System.Collections.Generic.List[int]
  foreach($p in $parts){
    if($p -match '^\d+$'){
      $v = [int]$p
      if($v -ge 1 -and $v -le $Max){ $idxs.Add($v-1) }
    } elseif($p -match '^\d+-\d+$'){
      $a,$b = $p -split '-'
      $a = [int]$a; $b = [int]$b
      if($a -gt $b){ $tmp=$a; $a=$b; $b=$tmp }
      for($z=$a; $z -le $b; $z++){ if($z -ge 1 -and $z -le $Max){ $idxs.Add($z-1) } }
    }
  }
...
function New-RandomPassword([int]$len=28){
  ...
  $chars = @(); foreach($s in $sets){ $chars += [char]($s | Get-Random) }
  while($chars.Count -lt $len){ $chars += [char](($sets | Get-Random) | Get-Random) }
  -join ($chars | Sort-Object {Get-Random})
}

function New-CredFromPlain([string]$User,[string]$PassPlain){
  ...
  return @{ Cred = (New-Object System.Management.Automation.PSCredential($User,$sec)); Plain = $PassPlain }
}

############ Infra ############
function Ensure-ResourceGroup([string]$Name,[string]$Loc){
  $rg = Get-AzResourceGroup -Name $Name -ErrorAction SilentlyContinue
  if(-not $rg){ $rg = New-AzResourceGroup -Name $Name -Location $Loc -Tag @{Company="PyxHealth";Workload="AVD";Env="Prod"} }
  $rg
}

function Ensure-Network([string]$Rg,[string]$Loc,[string]$VnetName,[string]$SubnetName,[string]$NsgName,[string]$OnPremCidr){
  ...
  if(-not $vnet){
    ...
    $vnet = New-AzVirtualNetwork -Name $VnetName -ResourceGroupName $Rg -Location $Loc -AddressPrefix "10.61.0.0/16" -Subnet $subCfg
  } elseif(-not ($vnet.Subnets | Where-Object Name -eq $SubnetName)){
    Add-AzVirtualNetworkSubnetConfig -Name $SubnetName -AddressPrefix "10.61.10.0/24" -VirtualNetwork $vnet | Set-AzVirtualNetwork | Out-Null
  ...
  if(-not $nsg){
    ...
    $nsg | Add-AzNetworkSecurityRuleConfig -Name "deny-rdp-internet" -Priority 1000 -Direction Inbound -Access Deny -Protocol Tcp `
      -SourceAddressPrefix Internet -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 3389 | Set-AzNetworkSecurityGroup | Out-Null
    # Allow RDP within VNet
    $nsg | Add-AzNetworkSecurityRuleConfig -Name "allow-rdp-virtualnetwork" -Priority 200 -Direction Inbound -Access Allow -Protocol Tcp `
      -SourceAddressPrefix "VirtualNetwork" -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 3389 | Set-AzNetworkSecurityGroup | Out-Null
    if($OnPremCidr){
      $nsg | Add-AzNetworkSecurityRuleConfig -Name "allow-rdp-onprem" -Priority 210 -Direction Inbound -Access Allow -Protocol Tcp `
        -SourceAddressPrefix $OnPremCidr -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 3389 | Set-AzNetworkSecurityGroup | Out-Null
    }
  } elseif($OnPremCidr -and -not ($nsg.SecurityRules | Where-Object {$_.Name -eq "allow-rdp-onprem"})){
    $nsg | Add-AzNetworkSecurityRuleConfig -Name "allow-rdp-onprem" -Priority 210 -Direction Inbound -Access Allow -Protocol Tcp `
      -SourceAddressPrefix $OnPremCidr -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 3389 | Set-AzNetworkSecurityGroup | Out-Null
  }
  ...
  if(-not $subnet.NetworkSecurityGroup -or $subnet.NetworkSecurityGroup.Id -ne $nsg.Id){
    Set-AzVirtualNetworkSubnetConfig -VirtualNetwork $vnet -Name $SubnetName -AddressPrefix $subnet.AddressPrefix -NetworkSecurityGroup $nsg | Out-Null
    $vnet | Set-AzVirtualNetwork | Out-Null
  }
  return @{ VNet=$vnet; Subnet=($vnet.Subnets | Where-Object Name -eq $SubnetName); Nsg=$nsg }
}

function Ensure-FSLogixStorage([string]$Rg,[string]$Loc,[string]$SaName,[string]$ShareName){
  Write-Color "Ensuring Storage + Files (FSLogix): $SaName / $ShareName" "INFO"
  $st = Get-AzStorageAccount -Name $SaName -ResourceGroupName $Rg -ErrorAction SilentlyContinue
  if(-not $st){
    $st = New-AzStorageAccount -ResourceGroupName $Rg -Name $SaName -Location $Loc -SkuName Standard_LRS -Kind StorageV2 `
  ...
  if(-not $share){ New-AzStorageShare -Name $ShareName -Context $ctx | Out-Null }
  $key = (Get-AzStorageAccountKey -ResourceGroupName $Rg -Name $SaName | Select-Object -First 1).Value
  return @{ Context=$ctx; ShareUNC="\\{0}.file.core.windows.net\{1}" -f $SaName,$ShareName; Key=$key }
}

############ AVD Core ############
function Ensure-AVDCore([string]$Rg,[string]$Loc,[string]$HpName,[string]$DagName,[string]$WsName,[string]$UserGroupObjectId){
  ...
  if(-not $hp){
    $hp = New-AzWvdHostPool -ResourceGroupName $Rg -Name $HpName -Location $Loc `
      -HostPoolType "Pooled" -LoadBalancerType "DepthFirst" -PreferredAppGroupType "Desktop" -MaxSessionLimit 12
  }
  ...
  if(-not $refs -or ($refs -notcontains $dag.Id)){
    Update-AzWvdWorkspace -ResourceGroupName $Rg -Name $WsName -ApplicationGroupReference (@($refs)+$dag.Id) | Out-Null
  }

  if($UserGroupObjectId){
    try{
      if(-not (Get-AzRoleAssignment -ObjectId $UserGroupObjectId -Scope $dag.Id -ErrorAction SilentlyContinue | Where-Object {$_.RoleDefinitionName -eq "Desktop Virtualization User"})){
        New-AzRoleAssignment -ObjectId $UserGroupObjectId -RoleDefinitionName "Desktop Virtualization User" -Scope $dag.Id | Out-Null
      }
      $rgScope = (Get-AzResourceGroup -Name $Rg).ResourceId
      if(-not (Get-AzRoleAssignment -ObjectId $UserGroupObjectId -Scope $rgScope -ErrorAction SilentlyContinue | Where-Object {$_.RoleDefinitionName -eq "Virtual Machine User Login"})){
        New-AzRoleAssignment -ObjectId $UserGroupObjectId -RoleDefinitionName "Virtual Machine User Login" -Scope $rgScope | Out-Null
      }
  ...
  return @{ HostPool=$hp; AppGroup=$dag; Workspace=$ws; Token=$reg.Token }
}
...
function Resolve-RegionAndImage([string]$PreferredLoc,[string]$Code){
  ...
  throw "No Windows 11 image SKUs found in US regions for requested intent '$Code'."
}

############ Quota / Size ############
function Get-SizeInfo([string]$Size){
  switch ($Size) {
    "Standard_D2s_v5"  { @{ family="standardDSv5Family";  vcpus=2 } }
    "Standard_D4s_v5"  { @{ family="standardDSv5Family";  vcpus=4 } }
    "Standard_D8s_v5"  { @{ family="standardDSv5Family";  vcpus=8 } }
    "Standard_D2as_v5" { @{ family="standardDASv5Family"; vcpus=2 } }
    "Standard_D4as_v5" { @{ family="standardDASv5Family"; vcpus=4 } }
    "Standard_D2s_v4"  { @{ family="standardDSv4Family";  vcpus=2 } }
    "Standard_D4s_v4"  { @{ family="standardDSv4Family";  vcpus=4 } }
    default { throw "Unknown or unsupported VM size: $Size" }
  }
}

function Get-FamilyQuotaRemaining([string]$Location,[string]$FamilyName){
...
function Resolve-SizeAndQuota([string]$PreferredSize,[int]$RequestedCount,[string]$Location){
  ...
  foreach($sz in $candidates){
    ...
    if ($remaining -ge $needed) {
      Write-Color ("Quota OK in {0}: {1} (remain {2} cores) → size {3}, count {4}" -f $Location,$info.family,$remaining,$sz,$RequestedCount) "SUCCESS"
    ...
    if ($maxHosts -gt 0) {
      ...
      Write-Color ("Not enough quota in {0}: {1} (remain {2} cores) for size {3}" -f $Location,$info.family,$remaining,$sz) "WARNING"
    }
  }
  if($best){
...
function Resolve-RegionSizeQuota([string]$PreferredLoc,[string]$ImageCode,[string]$PreferredSize,[int]$RequestedCount){
  $regionOrder = @($PreferredLoc,"westus","westus2","centralus","eastus2","eastus","southcentralus","northcentralus") | Select-Object -Unique
  foreach($loc in $regionOrder){
  ...
  throw "No US region has both (a) the requested image available and (b) enough VM-family quota to deploy even 1 host. Request a quota increase for Dsv5/Dasv5 (or edit the size list)."
}
...
function New-AvdVm {
  ...
  $nic = New-AzNetworkInterface -Name "$Name-nic" -ResourceGroupName $Rg -Location $Loc -SubnetId $Subnet.Id
  $vm = New-AzVMConfig -VMName $Name -VMSize $VmSize |
        Set-AzVMOperatingSystem -Windows -ComputerName $Name -Credential $AdminCred -ProvisionVMAgent -EnableAutoUpdate |
        Set-AzVMSourceImage -PublisherName $ResolvedImage.Publisher -Offer $ResolvedImage.Offer -Skus $ResolvedImage.Sku -Version $ResolvedImage.Version |
  ...
  New-AzVM -ResourceGroupName $Rg -Location $Loc -VM $vm -Verbose:$false | Out-Null
}
...
"@
  ...
  $settings = @{ "commandToExecute" = "powershell -ExecutionPolicy Bypass -EncodedCommand $b64" } | ConvertTo-Json
  Set-AzVMExtension -ResourceGroupName $Rg -VMName $VmName -Name "avd-join" `
    -Publisher "Microsoft.Compute" -ExtensionType "CustomScriptExtension" -TypeHandlerVersion "1.10" `
    -SettingString $settings | Out-Null
}

############ NEW: Interactive VM picker ############
$VmSizeMenu = @(
  "Standard_D2s_v5",
  "Standard_D4s_v5",
  "Standard_D8s_v5",
...
function Force-ChooseVmConfig {
  ...
  return @{ Count=$chosenCount; Size=$chosenSize }
}

############ Per-subscription deploy ############
function Deploy-ToSubscription {
  ...
  $abbr = Get-RegionAbbrev -Region $Loc; if(-not $abbr){ $abbr="PHE" }
  ...
  Ensure-ResourceGroup -Name $rgName -Loc $Loc | Out-Null
  ...
  if ($FirstVmThenPrompt -and $toDeploy -gt 1) {
    $toDeploy = 1
    Write-Color "FirstVmThenPrompt: deploying 1 VM first, then I'll ask about the rest." "INFO"
  }
  ...
  for($i=1; $i -le $toDeploy; $i++){
    ...
    Set-AvdAgentAndFslogix -Rg $rgName -VmName $vmName -RegistrationToken $avd.Token `
      -ConfigureFslogix:$DeployFslogix -FslogixUNC $fs.ShareUNC -StorageKey $fs.Key
  }

  if ($FirstVmThenPrompt -and $initial -gt 1) {
    $remaining = $initial - 1
    $ans = Read-Host "First VM is being created. Deploy the remaining $remaining host(s) now? (Y/N)"
    if ($ans -match '^[Yy]$') {
      ...
      Write-Color "Okay — leaving it with a single session host for now." "WARNING"
    }
  }
...
try{
  ...
  # 1) Subscription selection (unchanged)
  $subs = Force-ChooseSubscriptions

  # 2) NEW — VM interactive picker (count + size)
  $vmPick = Force-ChooseVmConfig
  $VmCountEffective = $vmPick.Count
  $VmSizeEffective  = $vmPick.Size

  foreach($s in $subs){
    try{
      Deploy-ToSubscription -Sub $s -Loc $Location -Count $VmCountEffective -PreferredSize $VmSizeEffective -Image $Image `
        -VmPrefix $VmPrefix -VmCred $vmCred -OnPremCidr $OnPremCidr -UserGroupObjectId $UserGroupObjectId `
        -DeployFslogix:$DeployFslogix -FirstVmThenPrompt:$FirstVmThenPrompt
    } catch {
  ...
  Write-Color "If blocked by quota everywhere, request increase for Dsv5/Dasv5 or adjust Resolve-SizeAndQuota size list." "WARNING"
  exit 1
}
