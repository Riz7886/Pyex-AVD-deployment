...
Write-Log "Auditing encryption settings..."

foreach ($storage in $inventory.Resources.StorageAccounts) {
    $severity = "Low"
    $issues = @()
    ...
    if (!$storage.Encryption.Services.Blob.Enabled) {
        $severity = "High"
        $issues += "Blob encryption disabled"
    }
    
    if (!$storage.Encryption.Services.File.Enabled) {
        $severity = "Medium"
        $issues += "File encryption disabled"
    }
    
    $issue = if ($issues.Count -gt 0) { $issues -join "; " } else { "Properly configured" }
    ...
    $findings += [PSCustomObject]@{
        ...
        BlobEncryption = $storage.Encryption.Services.Blob.Enabled
        FileEncryption = $storage.Encryption.Services.File.Enabled
        Severity = $severity
        Issue = $issue
...
foreach ($vm in $inventory.Resources.VMs) {
    ...
    $osDiskEncrypted = $vmDetails.StorageProfile.OsDisk.ManagedDisk.DiskEncryptionSet -ne $null
    
    $severity = if ($osDiskEncrypted) { "Low" } else { "High" }
    $issue = if ($osDiskEncrypted) { "OS disk encrypted" } else { "OS disk not encrypted" }
...
Write-Log "Found $($findings.Count) encryption findings" "SUCCESS"
Export-Report -Inventory $inventory -IdleResources $idle -Findings $findings -ReportName "3-Encryption-Audit" -Format $OutputFormat
