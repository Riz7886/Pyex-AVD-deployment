...
Write-Log "Auditing backup configuration..."

foreach ($vm in $inventory.Resources.VMs) {
    $backupEnabled = $false
    $vaultName = "None"
    
    try {
        $vmBackup = Get-AzRecoveryServicesBackupItem -BackupManagementType AzureVM -WorkloadType AzureVM -Name $vm.Name -ErrorAction SilentlyContinue
        if ($vmBackup) {
            $backupEnabled = $true
            $vaultName = $vmBackup.VaultName
        }
    } catch { }
    
    $severity = if ($backupEnabled) { "Low" } else { "High" }
    $issue = if ($backupEnabled) { "Backup configured" } else { "No backup configured" }
...
foreach ($sqlServer in $sqlServers) {
    ...
    foreach ($db in $databases) {
        $backupPolicy = $db.BackupRetentionPolicyState
        $backupEnabled = $backupPolicy -ne "Disabled"
        
        $severity = if ($backupEnabled) { "Low" } else { "Critical" }
        $issue = if ($backupEnabled) { "Backup retention: $($db.BackupRetentionPeriod) days" } else { "No backup configured" }
        
        $findings += [PSCustomObject]@{
            ResourceType = "SQL Database"
            Name = "$($sqlServer.ServerName)/$($db.DatabaseName)"
            ResourceGroup = $sqlServer.ResourceGroupName
            Location = $sqlServer.Location
            BackupEnabled = $backupEnabled
            RecoveryVault = "Built-in SQL backup"
            Severity = $severity
            Issue = $issue
            Subscription = $sub.Name
        }
...
Write-Log "Found $($findings.Count) backup findings" "SUCCESS"
Export-Report -Inventory $inventory -IdleResources $idle -Findings $findings -ReportName "4-Backup-Audit" -Format $OutputFormat
