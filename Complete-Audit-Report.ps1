...
foreach ($assignment in $roleAssignments) {
    $severity = if ($assignment.RoleDefinitionName -in @("Owner","Contributor","User Access Administrator")) { "High" } else { "Low" }
...
foreach ($nsg in $inventory.Resources.NetworkSecurityGroups) {
    foreach ($rule in $nsg.SecurityRules) {
        $severity = "Low"
        if ($rule.SourceAddressPrefix -in @("*","Internet","0.0.0.0/0") -and $rule.Access -eq "Allow") { $severity = "High" }
        $findings += [PSCustomObject]@{
...
foreach ($storage in $inventory.Resources.StorageAccounts) {
    $severity = if (!$storage.EnableHttpsTrafficOnly) { "High" } else { "Low" }
    $findings += [PSCustomObject]@{
        AuditArea = "Storage Security"
...
Export-ComprehensiveReport -Inventory $inventory -CostAnalysis $costAnalysis -DetailedFindings $findings -ReportName "Complete-Audit-Report" -Format $OutputFormat
