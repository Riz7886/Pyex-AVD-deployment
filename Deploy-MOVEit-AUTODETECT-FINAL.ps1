# ================================================================
# MOVEIT AZURE FRONT DOOR - AUTO-DETECT EVERYTHING
# NO HARDCODED NAMES - FINDS RESOURCES AUTOMATICALLY
# 100% AUTOMATED
# ================================================================

Write-Host "============================================" -ForegroundColor Cyan
Write-Host "  MOVEIT FRONT DOOR DEPLOYMENT" -ForegroundColor Cyan
Write-Host "  Auto-Detects All Resources" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
# ----------------------------------------------------------------
Write-Log "============================================" "Cyan"
Write-Log "AUTO-DETECTING RESOURCES" "Cyan"
Write-Log "============================================" "Cyan"
Write-Host ""

Write-Log "Looking for resource group rg-moveit..." "Yellow"
$rgExists = az group show --name "rg-moveit" 2>$null
if (-not $rgExists) {
    Write-Log "ERROR: rg-moveit not found!" "Red"
    Write-Log "Creating rg-moveit..." "Yellow"
    az group create --name "rg-moveit" --location "westus" --output none
    Write-Log "Created rg-moveit" "Green"
}

$resourceGroup = "rg-moveit"
$location = "westus"
Write-Log "Using Resource Group: $resourceGroup" "Green"

# ----------------------------------------------------------------
# STEP 5: AUTO-DETECT VNET
# ----------------------------------------------------------------
Write-Log "Looking for VNets in $resourceGroup..." "Yellow"
$vnets = az network vnet list --resource-group $resourceGroup --output json 2>$null | ConvertFrom-Json

if ($vnets -and $vnets.Count -gt 0) {
    $selectedVNet = $vnets[0].name
    Write-Log "Found VNet: $selectedVNet" "Green"
} else {
    Write-Log "No VNet found. Creating vnet-moveit..." "Yellow"
    az network vnet create --resource-group $resourceGroup --name "vnet-moveit" --location $location --address-prefixes "10.10.0.0/16" --output none
    $selectedVNet = "vnet-moveit"
    Write-Log "Created VNet: $selectedVNet" "Green"
}

# ----------------------------------------------------------------
# STEP 6: AUTO-DETECT SUBNET
# ----------------------------------------------------------------
Write-Log "Looking for Subnets in $selectedVNet..." "Yellow"
$subnets = az network vnet subnet list --resource-group $resourceGroup --vnet-name $selectedVNet --output json 2>$null | ConvertFrom-Json

if ($subnets -and $subnets.Count -gt 0) {
    $selectedSubnet = $subnets[0].name
    Write-Log "Found Subnet: $selectedSubnet" "Green"
} else {
    Write-Log "No Subnet found. Creating snet-moveit..." "Yellow"
    az network vnet subnet create --resource-group $resourceGroup --vnet-name $selectedVNet --name "snet-moveit" --address-prefixes "10.10.1.0/24" --output none
    $selectedSubnet = "snet-moveit"
    Write-Log "Created Subnet: $selectedSubnet" "Green"
}

# ----------------------------------------------------------------
# STEP 7: MOVEIT IP (HARDCODED)
# ----------------------------------------------------------------
$moveitIP = "192.168.0.5"
Write-Log "MOVEit Server IP: $moveitIP" "Green"
Write-Host ""
...
$config = @{
    ...
    Location                 = $location
    VNetName                 = $selectedVNet
    SubnetName               = $selectedSubnet
    MOVEitPrivateIP          = $moveitIP
...
Write-Log "============================================" "Cyan"
Write-Log "DEPLOYMENT CONFIGURATION" "Cyan"
Write-Log "============================================" "Cyan"
Write-Host ""
Write-Host "Resource Group: $($config.ResourceGroup)" -ForegroundColor Yellow
Write-Host "VNet: $($config.VNetName)" -ForegroundColor Yellow
Write-Host "Subnet: $($config.SubnetName)" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "Security Layers: 6" -ForegroundColor Green
Write-Host "  - Front Door (DDoS)" -ForegroundColor White
Write-Host "  - WAF (DefaultRuleSet 1.0)" -ForegroundColor White
Write-Host "  - Bot Manager" -ForegroundColor White
Write-Host "  - NSG" -ForegroundColor White
